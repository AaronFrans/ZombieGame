...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		{
			float highInfluence{ FLT_MIN };

			for (const auto& connection : GetNodeConnections(node))
			{
				auto connectionTo = GetNode(connection->GetTo());



				float influence{ connectionTo->GetInfluence() * expf(-connection->GetCost() * m_Decay)};

				if (abs(influence) > abs(highInfluence))
				{
					highInfluence = influence;
				}
			}

			float newInfluence{ Lerp(node->GetInfluence(), highInfluence, m_Momentum) };

			m_InfluenceDoubleBuffer[node->GetIndex()] = newInfluence;
