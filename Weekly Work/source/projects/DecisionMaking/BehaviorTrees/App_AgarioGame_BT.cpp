...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		Blackboard* pBB = CreateBlackboard(newAgent);
		//2. Create BehaviorTree

		BehaviorTree* pBT = new BehaviorTree(pBB, new BehaviorAction(BT_Actions::ChangeToWander));
	...
	//3. Set the BehaviorTree active on the agent 
	m_pSmartAgent->SetDecisionMaking(pBT);
